...
import { Loader2, Wand2, Copy, Check, ArrowLeft, Sparkles, User, Briefcase, Heart, Coins } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";
import { useNavigate } from "react-router-dom";

const BioGenerator = ({user}) => {
  ...
  const handleGenerate = async () => {
    if (!formData.hobbies || !formData.vibe) {
      toast({ title: "Missing Info", description: "Please fill in your hobbies and vibe.", variant: "destructive" });
      return;
    }
      ...
      <div className="container mx-auto py-12 px-4 max-w-5xl relative z-10">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Stop overthinking. Let AI craft the perfect introduction based on who you actually are.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2.5">
                  <Label htmlFor="hobbies" className="text-sm font-medium flex items-center gap-2">
                    <Heart className="h-4 w-4 text-rose-500" /> Hobbies & Interests
                  </Label>
                  <Textarea 
                    id="hobbies" 
                ...
                <div className="space-y-2.5">
                  <Label htmlFor="job" className="text-sm font-medium flex items-center gap-2">
                    <Briefcase className="h-4 w-4 text-blue-500" /> Job / Career <span className="text-muted-foreground text-xs font-normal ml-auto">(Optional)</span>
                  </Label>
                  <Input 
                    id="job" 
                ...
                <div className="space-y-2.5">
                  <Label htmlFor="vibe" className="text-sm font-medium flex items-center gap-2">
                ...
                >
                  {isLoading ? (
                    <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Crafting Persona...</>
                  ) : (
          ...
          <div className="space-y-6">
             <div className="flex items-center justify-between pb-2">
                ...
                {result && <span className="text-sm text-muted-foreground">Click to copy</span>}
             </div>
             ...
             {!result && !isLoading && (
               <div className="h-[450px] border-2 border-dashed border-muted-foreground/10 rounded-3xl flex flex-col items-center justify-center text-muted-foreground p-8 text-center bg-card/30 backdrop-blur-sm">
                 ...
                 <p className="max-w-xs text-sm opacity-80">Fill out the form and hit generate to see your new personas appear here.</p>
               </div>
             )}
             ...
             {isLoading && (
               <div className="space-y-4">
                 {[1, 2, 3].map((i) => (
                   <div key={i} className="h-32 bg-muted/20 animate-pulse rounded-2xl border border-muted/20" />
                 ))}
               </div>
             )}
                 ...
                 {result.bios.map((bio, index) => (
                   ...
                   >
                     ...
                     <div className={`absolute top-0 left-0 w-1.5 h-full transition-all duration-300 group-hover:w-2
                       ${bio.tone === 'Funny' ? 'bg-yellow-400' : 
                         bio.tone === 'Mysterious' ? 'bg-purple-500' : 'bg-blue-500'}`} 
                     />
                     
                     <CardContent className="pt-6 pl-8 pr-6 pb-6">
                       <div className="flex justify-between items-start mb-4">
                         <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border bg-background/50
                            ${bio.tone === 'Funny' ? 'text-yellow-600 border-yellow-200' : 
                              bio.tone === 'Mysterious' ? 'text-purple-600 border-purple-200' : 'text-blue-600 border-blue-200'}
                         `}>
                           {bio.tone}
                         </span>
                       ...
                       <p className="text-lg font-medium leading-relaxed text-foreground/90">"{bio.text}"</p>
                     </CardContent>
                   </Card>
                 ))}
               </div>
